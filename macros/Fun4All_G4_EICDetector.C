    ...
    const string &outdir = ".")
{
  ...
  //
	...
	cout << getenv("CALIBRATIONROOT")<<endl;
...
//   Enable::IP8 = true;
  ...
  if (Input::SIMPLE)
  {
    ...
    INPUTGENERATOR::SimpleEventGenerator[0]->set_eta_range(3.5,6.5);
    //INPUTGENERATOR::SimpleEventGenerator[0]->set_theta_range(.005,.02);
    INPUTGENERATOR::SimpleEventGenerator[0]->set_phi_range(-M_PI, M_PI);
    INPUTGENERATOR::SimpleEventGenerator[0]->set_p_range(1, 400.);
  }
  // Upsilons
...
//  Enable::TRACKING = true;
  Enable::B0TRACKING = true;
  Enable::TRACKING = false;
Enable::TRACKING_EVAL = Enable::TRACKING && true;
Enable::B0TRACKING_EVAL = Enable::B0TRACKING && true;
 ...
 // G4B0TRACKING::DISPLACED_VERTEX = false;  // this option exclude vertex in the track fitting and use RAVE to reconstruct primary and 2ndary vertexes
  G4B0TRACKING::DISPLACED_VERTEX = true;  // this option exclude vertex in the track fitting and use RAVE to reconstruct primary and 2ndary vertexes
...
//  G4TRACKING::PROJECTION_B0 = false;
  G4B0TRACKING::PROJECTION_B0 = false;
...
//  Enable::FWDJETS = true;
  ...
  // B0
   ...
   Enable::B0_FULLHITPLANE = false;

  Enable::B0ECALTOWERS = true; 	//To Construct B0 ECal
...
//   Enable::B0_TRACK=true; // for future tracking in Si layers
...
//   EVENT_EVALUATOR::Verbosity = 1;
  ...
  G4MAGNET::magfield = string("B0MagField_all_v2.root");
//  G4MAGNET::magfield = string("/cvmfs/eic.opensciencegrid.org/ecce/gcc-8.3/opt/fun4all/core/calibrations/Field/Map/sphenix3dbigmapxyz.root");
  ...
  if (Enable::B0ECAL_TOWER) B0ECAL_Towers();
  if (Enable::B0ECAL_CLUSTER) B0ECAL_Clusters();
  ...
  if (Enable::B0TRACKING) B0Tracking_Reco();
  ...
  if (Enable::B0TRACKING_EVAL) B0Tracking_Eval(outputroot + "_g4b0tracking_eval_mum_test_nd.root");
  ...
  if (Enable::B0ECAL_EVAL) B0ECAL_Eval(outputroot + "_g4b0ecal_eval_test.root");
