...
//#include <G4_B0TR.C> // for B0 Tracking
...
R__LOAD_LIBRARY(libEICG4B0ECAL.so)//
...
int G4Setup()
{
  ...
  {  // conversion to double fails -> we have a string
    ...
    if (G4MAGNET::magfield.find("B0MagField_all_v2.root") != string::npos)
