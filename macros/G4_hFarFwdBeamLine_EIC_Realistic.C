...
void hFarFwdDefineDetectorsIP6(PHG4Reco *g4Reco)
{
  ...
  for (int i = 0; i < rpDetNr; i++)
  {
    ...
    auto *detRP = new PHG4CylinderSubsystem(Form("rpTruth_%d", 2*i), 2*i);
...
/* *** Section for Realistic Roman Pots *** */
    auto *detRPe = new PHG4CylinderSubsystem(Form("rpTruth_%d", 2*i+1), 2*i+1);
    detRPe->SuperDetector("rpTruth");
    detRPe->set_double_param("place_x", PosFlip(rp_xCent[i]));
    detRPe->set_double_param("place_y", 0);
    detRPe->set_double_param("place_z", rp_zCent[i] - hFarFwdBeamLine::enclosure_center + .1015);
    detRPe->set_double_param("rot_y", AngleFlip(0.047 * TMath::RadToDeg()));
    detRPe->set_double_param("radius", 0);
    detRPe->set_double_param("thickness", 25);  // This is intentionally made large 25cm radius
    detRPe->set_double_param("length", 0.2);
    detRPe->set_string_param("material", "G4_Cu");
    detRPe->set_color(1,0,1,.8);
    detRPe->OverlapCheck(overlapCheck);
    detRPe->SetMotherSubsystem(hFarFwdBeamLine::hFarFwdBeamLineEnclosure);

    detRPe->SetActive();
    if (verbosity)
      detRPe->Verbosity(verbosity);
...
/* End */
  ...
  {
    auto *detB0 = new EICG4B0ZDCSubsystem(Form("b0Truth_%d", 2*i), 2*i);
...
//  detB0->set_int_param("ispipe", 0); //for future pipe implementation
    ...
    auto *detB0e = new EICG4B0ZDCSubsystem(Form("b0Truth_%d", 2*i+1), 2*i+1);
...
//  detB0e->set_int_param("ispipe", 0); //for future pipe implementation
  ...
  auto *B0Ecal = new EICG4B0ZDCSubsystem("B0Ecal", 2*b0DetNr);
...
//  B0Ecal->set_int_param("ispipe", 0); //for future pipe implementation
  ...
  auto *B0Ecale = new EICG4B0ZDCSubsystem("B0Ecale", 2*b0DetNr + 1);
  B0Ecal->SuperDetector("b0Truth");
...
/* Beam Pipe in B0 *//*
  auto *B0pipe = new EICG4B0ZDCSubsystem("B0beamPipe", 2*b0DetNr + 2);
	B0pipe->SuperDetector("b0Truth");
  B0pipe->set_int_param("ispipe", 1);
  B0pipe->set_double_param("pipe_ir", 2.8);
  B0pipe->set_double_param("pipe_or", 3.05);
  B0pipe->set_double_param("length", 195);
  B0pipe->set_string_param("material", "G4_Al");
  B0pipe->set_double_param("pipe_x", -3.4);
  B0pipe->set_double_param("pipe_y", 0);
  B0pipe->set_double_param("pipe_z", 0);
  B0pipe->set_double_param("detid",2*b0DetNr+2);
  B0pipe->SetActive(true);
  if (verbosity)
	  B0pipe->Verbosity(verbosity);
  B0pipe->OverlapCheck(overlapCheck);
  B0pipe->SetMotherSubsystem(hFarFwdBeamLine::B0Magnet);
  g4Reco->registerSubsystem(B0pipe); */
}
