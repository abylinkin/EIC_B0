// - 1/June/2021 TTree production for B0ZDC G4 hits     Shima Shimizu
//
...
EICG4B0ZDCHitTree::EICG4B0ZDCHitTree(const std::string &name, const std::string &filename)
    : SubsysReco(name)
    , _filename(filename)
    , tree(nullptr)
    , outfile(nullptr)
...
EICG4B0ZDCHitTree::~EICG4B0ZDCHitTree(){
}

int EICG4B0ZDCHitTree::Init(PHCompositeNode *)
{
...
int EICG4B0ZDCHitTree::process_event(PHCompositeNode *topNode)
{
...
int EICG4B0ZDCHitTree::End(PHCompositeNode *topNode)
{
    outfile->cd();
    tree->Write();
    outfile->Write();
...
void EICG4B0ZDCHitTree::AddNode(const std::string &name, const int detid)
{
