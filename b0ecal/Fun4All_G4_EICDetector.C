...
//   Enable::IP8 = true;
  ...
  if (Input::SIMPLE)
  {
    ...
    INPUTGENERATOR::SimpleEventGenerator[0]->set_eta_range(3,6.5);
    INPUTGENERATOR::SimpleEventGenerator[0]->set_phi_range(-M_PI, M_PI);
    INPUTGENERATOR::SimpleEventGenerator[0]->set_pt_range(1, 50.);
  }
  // Upsilons
...
//  Enable::FWDJETS = true;
  ...
  if (Enable::B0ECAL_EVAL) B0ECAL_Eval(outputroot + "_g4b0ecal_eval.root");
...
/*  if (Enable::B0_HITTREE)
  {
     EICG4B0HitTree *b0hit = new EICG4B0HitTree("Hits");
     b0hit->AddNode("G4HIT_b0Truth_0", 0);
     se->registerSubsystem(b0hit);
  }
*/  //-----------------
  // Event processing
  //-----------------
